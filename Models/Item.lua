...
function Self:GetThresholdForLocation(unit, upper)
    ...
    if not upper and self:IsArmorToken() then
        ...
        if Util.Select(self:GetBasicInfo().equipLoc, Self.TYPE_TRINKET, thresholdTrinket, Self.TYPE_FINGER, thresholdRings) then
...
function Self:GetEquippedForLocation(unit)
    ...
    elseif self.isEquippable then
        ...
        for _, slot in pairs(Self.SLOTS[self.equipLoc]) do
...
function Self:GetPosition(refresh)
    ...
    -- Check equipment
    if select(2, self:GetBasicInfo()) and self.isEquippable and not self:IsRelic() then
